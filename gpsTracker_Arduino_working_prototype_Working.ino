...
void loop()
{
  ...
  char* buf = x.c_str();
 // no3[32], Speed[32], no4[32], no5[32], no6[32], no7[32], no8[32], no9[32], no10[32], Satt[32];
  ...
  strcpy(no3 , strtok(NULL, ","));
  strcpy(Speed, strtok(NULL, ","));
  strcpy(no4 , strtok(NULL, ","));
  strcpy(no5 , strtok(NULL, ","));
  strcpy(no6 , strtok(NULL, ","));
  strcpy(no7 , strtok(NULL, ","));
  strcpy(no8 , strtok(NULL, ","));
  strcpy(Satt, strtok(NULL, ","));
  
  Serial.print("Date/Time: "); Serial.println(date);
  Serial.print("Latitude : "); Serial.println(lat);
  Serial.print("Longitude: "); Serial.println(lon);
  Serial.print("Speed :"); Serial.println(Speed);
  Serial.print("Nbr_Satt :"); Serial.println(Satt);
  Serial.println("");
  sendGprsData();
}
...
void sendMQTTMessage(char* clientId, char* brokerUrl, char* brokerPort, char* topic, char* message)
{
  ...
  delay(2000);
  mySerial.println("AT+CIFSR"); //get IP adresse
  delay(1000);
  ...
  Serial.println(atCommand);  // Serial.println("AT+CIPSTART=\"TCP\",\"mqttdashboard.com\",\"1883\"");
  delay(1000);
  ...
  for (int k = 0; k < mqttMessageLength; k++)
  {
    ...
    //Serial.print(mqttMessage[k]); // Message contents
    //Serial.println("");
  }
  mySerial.write(byte(26)); // (signals end of message)
  Serial.println("");
  Serial.println("------------- MQTT Publish mmessage -------------"); // Message contents
  delay(2000);
